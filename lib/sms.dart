...
export 'package:sms_v2/contact.dart';
...
class SmsThread {
  ...
  Future findContact() async {
    ...
    var contact = await query.queryContact(_address.toString());
    if (contact != null) {
...
class SmsSender {
  ...
  SmsSender._private(this._channel, this._stateChannel) : _sentMessages = {} {
  ...
  void _onSmsStateChanged(dynamic stateChange) {
    ...
    if (_sentMessages.containsKey(id)) {
      final message = _sentMessages[id]!;
        ...
        case 'delivered':
          {
            ...
            _deliveredStreamController.add(message);
            _sentMessages.remove(id);
            break;
          }
        case 'fail':
          {
            message.state = SmsMessageState.Fail;
            _sentMessages.remove(id);
            break;
          }
      }
...
class SmsQuery {
  ...
  factory SmsQuery() {
    if (_instance == null) {
      final MethodChannel methodChannel = const MethodChannel(
        "plugins.babariviere.com/querySMS",
        const JSONMethodCodec(),
      );
